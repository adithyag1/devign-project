...
def bigvul(minimal=True, sample=False, return_raw=False, splits="default"):
    ...
    if minimal:
        try:
            df = pd.read_parquet(
                savedir / f"minimal_bigvul_{sample}.pq",
                engine="fastparquet"
            ).dropna()
    ...
    dfv = dfv[
        ~dfv.apply(
            lambda x: len(x.added) == 0 and len(x.removed) == 0,
            axis=1
        )
    ]
    ...
    dfv = dfv[
        ~dfv.apply(
            lambda x:
            x.func_before.strip()[-1] != "}" and
            x.func_before.strip()[-1] != ";",
            axis=1
        )
    ]
    ...
    dfv = dfv[
        ~dfv.before.apply(lambda x: x[-2:] == ");")
    ]

    dfv["mod_prop"] = dfv.apply(
        lambda x:
        len(x.added + x.removed) /
        len(x["diff"].splitlines()),
        axis=1
    )
    ...
    dfv = dfv[
        dfv.apply(
            lambda x: len(x.before.splitlines()) > 5,
            axis=1
        )
    ]
    ...
    df[metadata_cols].to_csv(
        utils.cache_dir() / "bigvul/bigvul_metadata.csv",
        index=0
    )
