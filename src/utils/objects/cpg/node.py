...
class Node:
    def __init__(self, node, indentation):
        self.raw_node = node
        self.id = str(node["id"]).split(".")[-1]
        self.label = self.id.split("@")[0]
        self.indentation = indentation + 1
        
        # Use .get() to avoid KeyError if properties are missing
        props_data = node.get("properties", {}) 
        self.properties = Properties(props_data, self.indentation)
        
        # Same for edges - the root might not have outgoing edges in the same way
        edges_data = node.get("edges", [])
        self.edges = {str(edge["id"]).split(".")[-1]: Edge(edge, self.indentation) for edge in edges_data}
