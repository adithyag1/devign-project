...
def select(dataset):
    ...
    vulnerable = result[result['target'] == 1].sample(n=min(len(result[result['target'] == 1],random_state=42), 500))
    non_vulnerable = result[result['target'] == 0].sample(n=min(len(result[result['target'] == 0],random_state=42), 500))
